...
- Подсветка синтаксиса в code-blocks (highlight.js): ```csharp перед блоком задаёт язык (или выбор в селекте при наведении), работает и в публичном просмотре
...
- 72 unit-теста (`Paages.Tests`) на SQLite in-memory, CI: build → migrations check → test
